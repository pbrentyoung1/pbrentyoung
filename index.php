<?php
require_once __DIR__ . '/inc/blog.php';
?>
<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Brent Young, Creative Director</title>
  <meta name="description" content="Brent Young helps churches and mission-driven organizations tell the right story clearly: brand, communications, creative leadership, and the systems that hold them all together.">
  <meta name="author" content="Brent Young">
  <meta name="theme-color" content="#f4f1ea">
  <link rel="canonical" href="https://pbrentyoung.com/">

  <!-- Google tag (gtag.js) -->
  <script async src="https://www.googletagmanager.com/gtag/js?id=G-KKQYWP70WN"></script>
  <script>
    window.dataLayer = window.dataLayer || [];
    function gtag(){dataLayer.push(arguments);}
    gtag('js', new Date());

    gtag('config', 'G-KKQYWP70WN');
  </script>

  <!-- Icons -->
  <link rel="icon" href="favicon.ico" sizes="48x48">
  <link rel="icon" type="image/png" sizes="32x32" href="favicon-32.png">
  <link rel="apple-touch-icon" href="apple-touch-icon.png">
  <link rel="manifest" href="site.webmanifest">

  <!-- Open Graph -->
  <meta property="og:type" content="website">
  <meta property="og:site_name" content="Brent Young">
  <meta property="og:title" content="Brent Young, Creative Director">
  <meta property="og:description" content="The right story, told the right way. Brand, communications, and creative leadership for churches and mission-driven organizations.">
  <meta property="og:url" content="https://pbrentyoung.com/">
  <meta property="og:image" content="https://pbrentyoung.com/images/og-image.png">
  <meta property="og:image:secure_url" content="https://pbrentyoung.com/images/og-image.png">
  <meta property="og:image:type" content="image/png">
  <meta property="og:image:width" content="1200">
  <meta property="og:image:height" content="630">
  <meta property="og:image:alt" content="Brent Young, Creative Director. The right story, told the right way.">
  <meta property="og:locale" content="en_US">

  <!-- Twitter -->
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:url" content="https://pbrentyoung.com/">
  <meta name="twitter:title" content="Brent Young, Creative Director">
  <meta name="twitter:description" content="The right story, told the right way. Brand, communications, and creative leadership for churches and mission-driven organizations.">
  <meta name="twitter:image" content="https://pbrentyoung.com/images/og-image.png">
  <meta name="twitter:image:alt" content="Brent Young, Creative Director. The right story, told the right way.">

  <script type="application/ld+json">
  {
    "@context": "https://schema.org",
    "@type": "Person",
    "name": "Brent Young",
    "url": "https://pbrentyoung.com/",
    "jobTitle": "Creative Director",
    "description": "Brent Young helps churches and mission-driven organizations tell the right story clearly: brand, communications, creative leadership, and the systems that hold them all together.",
    "address": {
      "@type": "PostalAddress",
      "addressLocality": "Montgomery",
      "addressRegion": "TX",
      "addressCountry": "US"
    },
    "knowsAbout": ["Brand strategy", "Church communications", "Creative direction", "Storytelling", "Design systems"]
  }
  </script>

  <link rel="stylesheet" href="css/editorial.css">
</head>

<body>
  <?php blog_site_header(); ?>

  <main id="top">
    <!-- ================= HERO / THE BOARD ================= -->
    <section class="hero board" id="hero" aria-label="Introduction">
      <div class="guides" aria-hidden="true">
        <div class="gv" style="left: 4vw;"></div>
        <div class="gv" style="right: 4vw;"></div>
        <div class="gh" style="top: 92px;"></div>
        <div class="gh" style="bottom: 70px;"></div>
        <span class="crop tl"></span><span class="crop tr"></span>
        <span class="crop bl"></span><span class="crop br"></span>
        <span class="reg"></span>
      </div>

      <div class="wrap hero-grid">
        <div class="p-in p2">
          <span class="kicker">Proof no. 2 &middot; Montgomery, Texas</span>
          <h1>
            The right story,
            <span class="edit-wrap">told&nbsp;<span class="edit-old">good.<span class="strike"></span></span><span class="edit-new">the right way.</span><span class="edit-note"><span class="caret">&#8963;</span>the right way</span></span>
          </h1>
          <p class="hero-sub" style="margin-bottom: 5px;">
            Every church has a God-given story worth telling.</p><p class="hero-sub" style="margin-bottom: 5px;margin-top:5px;">I help churches and mission-driven
            organizations steward that story through communication, brand, design, and the systems
            that hold them together.</p><p class="hero-sub" style="margin-top:5px;">The goal is alignment between what you believe, what you say,
            and what people experience.
          </p>
          <div class="hero-actions">
            <a class="btn-ink" href="#flat-file">Explore the file</a>
            <a class="btn-ghost" href="/blog">Read the blog</a>
          </div>
        </div>

        <div class="p-in p3">
          <div class="hero-portrait fpo" id="heroPortrait">
            <span class="fpo-tag">FPO</span>
            <img data-src="assets/img/thumbs/brent-hero-closer-thumb.webp" alt="Brent Young">
            <span class="hero-note" aria-hidden="true">final art &middot; ok &#10003;</span>
          </div>
        </div>
      </div>

      <div class="wrap hero-slug p-in p4">
        <div class="slugline">
          <span>JOB NO. BY-2026 &middot; COMMUNICATIONS &amp; CREATIVE ARTS</span>
          <span>NON-REPRO BLUE &middot; DO NOT PRINT</span>
        </div>
      </div>
    </section>

    <!-- ================= THE PREMISE (philosophy) ================= -->
    <section class="wrap" id="premise" aria-label="Philosophy" style="margin-top: 64px;">
      <div class="dept-head">
        <div class="dept-row">
          <h2>Before the work</h2>
          <span class="dept-no">DEPT. 00 &middot; BEFORE THE WORK BEGINS</span>
        </div>
      </div>
      <div class="premise-grid">
        <div class="sheet premise-sheet">
          <p class="hero-sub" style="margin-bottom: 14px;">
            The work begins before the brief, the first sketch, or the first frame of video.
            It begins with the story God has given you to tell and the people He is calling
            you to serve.
          </p>
          <p class="hero-sub" style="margin-bottom: 14px;">
            Long before someone walks through the front door or meets the pastor, they have
            already begun to experience your church. As they drive by, visit your website,
            watch a video, encounter a social post, or receive an email, every touchpoint
            tells them who you are and whether a place has been prepared for them.
          </p>
          <p class="hero-sub" style="margin: 0 0 26px;">
            Communication is one of the ways we begin caring for people before we know who
            they are. Good design prepares a place for them. Clear systems help us keep that
            promise as the work grows.
          </p>
        </div>
        <div class="premise-note">
          <p>
            The goal is not simply better-looking work. It is alignment between what you
            believe, what you say, and what people experience.
          </p>
          <p class="note-tag">THE RIGHT STORY, TOLD THE RIGHT WAY.</p>
        </div>
      </div>
    </section>

    <!-- ================= FIRST PRINCIPLES ================= -->
    <section class="wrap principles" id="principles" aria-label="First principles">
      <div class="dept-head">
        <div class="dept-row">
          <h2>First principles</h2>
          <span class="dept-no">DEPT. 01 &middot; THE CONVICTIONS</span>
        </div>
      </div>
      <p class="hero-sub" style="margin-bottom: 0;">
        Every project, essay, and framework here flows from a short list of convictions.
        These aren&rsquo;t taglines. They&rsquo;re the questions every decision has to answer.
        The work below is what they look like in practice.
      </p>
      <div class="principle-grid">
        <article class="pcard">
          <img class="pcard-cue" src="assets/img/principles/communication-is-stewardship.png" alt="" width="520" height="226">
          <h3 class="pcard-title">Communication is stewardship</h3>
          <p class="pcard-text">We care for each touchpoint because each one shapes how people experience the mission.</p>
        </article>
        <article class="pcard">
          <img class="pcard-cue" src="assets/img/principles/design-solves-creative-problems.png" alt="" width="520" height="284">
          <h3 class="pcard-title">Design solves creative problems</h3>
          <p class="pcard-text">Good design removes unnecessary work and prepares a place for people.</p>
        </article>
        <article class="pcard">
          <img class="pcard-cue" src="assets/img/principles/brand-builds-trust.png" alt="" width="520" height="248">
          <h3 class="pcard-title">Brand builds trust</h3>
          <p class="pcard-text">The mission declares the story. The experience confirms or contradicts it.</p>
        </article>
        <article class="pcard">
          <img class="pcard-cue" src="assets/img/principles/brand-is-the-referee.png" alt="" width="520" height="386">
          <h3 class="pcard-title">Brand is the referee</h3>
          <p class="pcard-text">Brand gives a team a shared standard for making decisions together.</p>
        </article>
        <article class="pcard">
          <img class="pcard-cue" src="assets/img/principles/story-before-strategy.png" alt="" width="520" height="207">
          <h3 class="pcard-title">Story before strategy</h3>
          <p class="pcard-text">Decide what story people should experience before choosing tactics.</p>
        </article>
        <article class="pcard">
          <img class="pcard-cue" src="assets/img/principles/systems-create-creativity.png" alt="" width="520" height="396">
          <h3 class="pcard-title">Systems create creativity</h3>
          <p class="pcard-text">Healthy systems give creative people room to think, make, collaborate, and rest.</p>
        </article>
        <article class="pcard">
          <img class="pcard-cue" src="assets/img/principles/mission-before-preference.png" alt="" width="520" height="347">
          <h3 class="pcard-title">Mission before preference</h3>
          <p class="pcard-text">Preferences have a place at the table, but mission makes the final call.</p>
        </article>
        <article class="pcard">
          <img class="pcard-cue" src="assets/img/principles/stewardship-isnt-what-you-protect.png" alt="" width="500" height="520">
          <h3 class="pcard-title">Stewardship is what you invest</h3>
          <p class="pcard-text">We steward the story by telling it clearly and investing what we have been given.</p>
        </article>
      </div>
      <div class="slugline" style="margin-top: 26px;">
        <span>SOURCE: DOCS/FIRST_PRINCIPLES.MD</span>
        <span>PROTECT &middot; EXPAND &middot; CHALLENGE</span>
      </div>
    </section>

    <!-- ================= THE FILE ================= -->
    <section class="flatfile slide-table" id="flat-file" aria-label="Project file">
      <div class="wrap">
        <div class="dept-head">
          <div class="dept-row">
            <h2>The file</h2>
            <span class="dept-no">DEPT. 02 &middot; PROJECTS &amp; CASE FILES</span>
          </div>
        </div>
        <p class="hero-sub" style="margin-bottom: 10px;">
          This is the work behind the convictions. Each piece began with a story to clarify,
          a person to serve, or a system that needed to work better. Open a file to see what
          was made. Records marked <strong>Case File</strong> go further, showing the problem,
          the decisions, the outcome, and what the work taught me.
        </p>
        <div class="flatfile-sub">
          <p class="file-count" id="fileCount" aria-live="polite"></p>
        </div>
        <div class="tabrow" id="tabs" role="tablist" aria-label="Filter by discipline"></div>
        <div class="filegrid" id="filegrid"></div>
        <button class="pullmore" id="pullMore" type="button" style="display:none;"></button>
      </div>
    </section>

    <!-- ================= FIELD NOTES ================= -->
    <section class="field-notes wrap" id="desk" aria-label="Field Notes" hidden>
      <div class="dept-head">
        <div class="dept-row">
          <div><span class="kicker">The Blog</span><h2>Field Notes</h2></div>
          <span class="dept-no">DEPT. 03 &middot; IDEAS IN PRACTICE</span>
        </div>
      </div>
      <p class="hero-sub" style="margin-bottom: 0;">
        Notes on communication, brand, design, creative leadership, and the systems that
        hold them together. They come from more than twenty years of making the work,
        leading teams, solving problems, and learning that clarity is one of the ways we
        care for people.
      </p>
      <div class="field-notes-grid">
        <div id="deskFeature"></div>
        <aside id="deskShortlist" aria-label="The Short List"></aside>
      </div>
      <a class="pullmore desk-all" href="/blog">READ ALL FIELD NOTES &rarr;</a>
    </section>

    <!-- ================= CONTACT ================= -->
    <section class="contact" id="contact" aria-label="Contact">
      <div class="wrap">
        <div class="dept-head">
          <div class="dept-row">
            <h2>Let&rsquo;s tell the right story.</h2>
            <span class="dept-no">DEPT. 04 &middot; FINAL PROOF</span>
          </div>
        </div>
        <p class="contact-intro">
          Maybe your story is clear, but the way people experience it is not. Maybe your
          team is doing good work inside systems that make everything harder than it needs
          to be. Or maybe you know something needs to change, but you are not sure where to
          begin. I would love to hear what you are working through. No pitch and no packages,
          just a conversation about the story God has given you to tell and how we might
          help people experience it more clearly.
        </p>
        <div class="contact-grid">
          <div class="contact-info">
            <p><strong>Brent Young</strong> &middot; Montgomery, Texas</p>
            <p class="mono">PHONE: (562) 964-4562</p>
            <p class="mono">EMAIL: PBRENTYOUNG@GMAIL.COM</p>
          </div>
          <div class="hero-actions">
            <a class="btn-ink" href="mailto:pbrentyoung@gmail.com">Email Brent</a>
            <a class="btn-ghost" href="assets/pdf/resume.pdf" target="_blank" rel="noopener">Resume</a>
          </div>
        </div>
        <div class="slugline" style="margin-top: 36px;">
          <span>JOB NO. BY-2026 &middot; APPROVED FOR RELEASE</span>
          <span>OK &#10003;</span>
        </div>
      </div>
    </section>
  </main>

  <?php blog_site_footer(true); ?>

  <!-- ================= JOB JACKET ================= -->
  <div class="jacket-layer" id="jacketLayer" aria-hidden="false">
    <div class="jacket-backdrop"></div>
    <div class="jacket" role="dialog" aria-modal="true" aria-label="Project details">
      <div class="jacket-bar">
        <span id="jkNo">JOB JACKET</span>
        <span class="controls">
          <button id="jkPrev" type="button" aria-label="Previous project">&larr;</button>
          <span id="jkPos"></span>
          <button id="jkNext" type="button" aria-label="Next project">&rarr;</button>
          <button id="jkClose" type="button" aria-label="Close">&times;</button>
        </span>
      </div>
      <div class="jacket-body">
        <div class="jacket-media" id="jkMedia"></div>
        <h3 class="jacket-title" id="jkTitle"></h3>
        <div class="jacket-codes" id="jkCodes"></div>
        <dl class="jacket-meta" id="jkMeta"></dl>
      </div>
      <div class="jacket-foot">
        <button class="share" id="jkRefile" type="button" style="color: var(--manila-mute);">RE-FILE (ESC)</button>
        <button class="share" id="jkShare" type="button"></button>
      </div>
    </div>
  </div>

  <script src="js/column-core.js"></script>
  <script src="js/editorial.js"></script>
</body>
</html>
