# Content Roadmap

## How This Roadmap Works

The pillars below are the canonical content taxonomy for the site.

Articles carry a `topic:` value in their frontmatter that must match a pillar name exactly: `Brand & Mission`, `Creative Leadership`, `Systems & Workflow`, `Craft`, or `AI`.

Two special cases:

- **Manifesto** is a reserved topic for cornerstone essays — starting with *The Right Story, Told the Right Way*, which serves as the manifesto for the website, the future book *Evangelistic Marketing*, and the Creative platform.
- **Case Studies** is a pillar that lives in the portfolio (`data/portfolio.json`), not the blog. Every case study should name the principle it illustrates and link to the article that explains it.

Titles marked ✍️ have working drafts in `posts/` (written with AI assistance; edit into Brent's voice before treating as published).

---

## Pillar 1

# Brand & Mission

- The Right Story, Told the Right Way ✍️ *(cornerstone — topic: Manifesto)*
- Your Church Already Has a Brand ✍️
- Brand Is the Referee ✍️
- The Most Expensive Word in Church Communications ✍️
- Marketing Isn't About Selling Jesus
- Every Church Has a Front Door
- Every Pixel Is Hospitality
- Your Church Isn't for Everyone *(companion to A Persona Is More Than a Demographic)*
  - Thesis: The gospel is for everyone. Your church is for someone.
  - Scripture spine: Galatians 2:7-9 (Peter to the circumcised, Paul to the Gentiles, the same gospel deliberately assigned to different audiences); Paul adapting per audience: synagogue history at Antioch (Acts 13), rain and crops at Lystra (Acts 14), altars and poets at Athens (Acts 17); tender at Thessalonica, rhetorical at Corinth; "all things to all people" (1 Corinthians 9:19-23).
  - Related idea captured with it: the funnel is a description, not a strategy. Jesus described the narrowing himself (the sower's four soils, "many are called, few are chosen"). The shape is not the problem; treating the people inside it as merely prospects is the problem. Could live here or as a sidebar in the persona article.
  - Guardrail: "not for everyone" describes calling and communication focus, never a door policy. Close the exclusion reading explicitly (the homogeneous unit principle went ugly places). Paul adapted his preaching at Athens; he turned no one away at Lystra. Specialization of voice, universality of welcome.

---

## Pillar 2

# Creative Leadership

- Preference vs Brand
- Change vs Polish ✍️ *(merged in You Can't Polish Your Way Into Relevance)*
- Giving Better Feedback
- The Creative Brief Is Ministry ✍️
- Volunteers Don't Need Better Software
- Creative Teams Need Better Systems

---

## Pillar 3

# Systems & Workflow

- Why Church Communications Feel Chaotic ✍️
- Stop Calling Everything a Project ✍️
- Campaign Thinking
- Every Deliverable Has One Job ✍️
- Communication Debt
- Systems Before Software

---

## Pillar 4

# Craft

- Why Beautiful Design Builds Trust
- Better Church Video
- Photography That Builds Trust
- Typography Is Theology
- Story Before Strategy

---

## Pillar 5

# Case Studies

Forge Church

Native American Workbook

Creative Platform

Wired In

Soul Sessions

Future projects

---

## Pillar 6

# AI

AI Can Generate Images. Design Solves Problems.

Prompting Isn't Design.

The Future Creative Director.

The Human Advantage.

Design Thinking in the Age of AI.

---

# Long-Term Projects

Evangelistic Marketing

Creative Platform

Speaking

Workshops

Courses

Podcast

Photography Essays