...
Optional frontmatter: `banner:` (overrides the default banner path), `bannerAlt:` (alt text for the banner), `toc: overview` (keeps numbered list entries out of the contents navigation), `draft: true` (hides the post), `featured: true` (selects the lead article), `featuredExcerpt:` (adds supporting copy to the featured treatment on `/blog`), and `shortlist:` (a positive number selects and orders the post in The Short List).
...
**Footnotes.** Use Markdown footnotes when a source should substantiate a claim without interrupting the article's voice. Place `[^1]` immediately after the supported sentence and add the matching `[^1]: Source note` definition at the end of the Markdown file. Keep footnotes concise, identify informal or limited evidence honestly, and link to the primary source when possible.
